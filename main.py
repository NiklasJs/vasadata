...
def read_data(available_years):
    """Load and concatenate parquet files for multiple years efficiently."""
...
CONTROL_POINTS = ["Start", "High Point", "Smågan", "Mångsbodarna", "Risberg", "Evertsberg", "Oxberg", "Hökberg", "Eldris", "Finish"]
NUMERIC_COLS = ["placement", "placement_gender", "year", "duration_s", "duration_m", "duration_h", "d_duration_s", "d_duration_m","height_m", "distance_km", "d_distance_km", "d_ascent", "d_descent", "avg_speed_kmh", "avg_speed_minkm"]
CATEGORICAL_COLS = ["class", "club", "gender", "country"]
STRING_COLS = ["startnr", "name", "name_startnr"]
MAX_SPEED_KMPH = 40

@st.cache_data
def prepare_dataframe(df):
    """Convert columns to appropriate data types - run once per dataset."""
    df = df.copy()
    df[NUMERIC_COLS] = df[NUMERIC_COLS].apply(pd.to_numeric)
    df[CATEGORICAL_COLS] = df[CATEGORICAL_COLS].astype("category")
    df[STRING_COLS] = df[STRING_COLS].astype("string")
    df["startgroup"] = pd.Categorical(df["startgroup"], categories=STARTGROUPS, ordered=True)
    df["control"] = pd.Categorical(df["control"], categories=CONTROL_POINTS, ordered=True)
    return df

if 'data' not in st.session_state:
    st.session_state['data'] = prepare_dataframe(read_data(available_years=AVAILABLE_YEARS))
...
# Filter once per year
mask_year = df_full.year == year
df = df_full[mask_year]

first_controlpoint = "High Point" if year >= 2022 else "Smågan"

# Create boolean masks to avoid repeated filtering
mask_finish = df["control"] == "Finish"
mask_female = df["gender"] == "W"
mask_male = df["gender"] == "M"

# Calculations - minimize filtering
start_participants = df["startnr"].nunique()  # More efficient than len(unique())
...
# Process finish data once
df_finish = df[mask_finish].copy()
male_times = df_finish[mask_male]["duration_s"]
female_times = df_finish[mask_female]["duration_s"]
male_medal_threshold = male_times.min() * 1.5 if len(male_times) > 0 else 0
female_medal_threshold = female_times.min() * 1.5 if len(female_times) > 0 else 0

male_medalists = ((df_finish[mask_male]["duration_s"] <= male_medal_threshold).sum())
female_medalists = ((df_finish[mask_female]["duration_s"] <= female_medal_threshold).sum())

df_finish_medal = df_finish[["startgroup", "gender"]].groupby("startgroup").size().reset_index(name="participants")
...
# Reuse masks and minimize filtering
finish_participants = df[mask_finish]["startnr"].nunique()
females_finishing = df[mask_finish & mask_female]["startnr"].nunique()
males_finishing = df[mask_finish & mask_male]["startnr"].nunique()

# Calculate breakoffs efficiently
control_points = CONTROL_POINTS[1:]  # Exclude "Start" for breakoff analysis
grp_female = df[mask_female].groupby("control")["startnr"].nunique().reindex(CONTROL_POINTS)
grp_female.loc["Start"] = females
female_breaks = -(grp_female - grp_female.shift(1))

grp_male = df[mask_male].groupby("control")["startnr"].nunique().reindex(CONTROL_POINTS)
grp_male.loc["Start"] = males
male_breaks = -(grp_male - grp_male.shift(1))

# Create aggregated break data
df_breaks = pd.DataFrame({
    "control": female_breaks.index,
    "Total": female_breaks.values + male_breaks.values
}).iloc[1:]  # Skip Start row

df_breaks_share = pd.DataFrame({
    "control": female_breaks.index,
    "W": 100 * female_breaks.values / female_breaks.sum(),
    "M": 100 * male_breaks.values / male_breaks.sum()
}).iloc[1:].melt("control", var_name="variable", value_name="value")

# Calculate breakoffs per start group
row_start = df.groupby("startgroup")["startnr"].nunique().reindex(STARTGROUPS)
row_finish = df[mask_finish].groupby("startgroup")["startnr"].nunique().reindex(STARTGROUPS)
row_breakoff = row_start - row_finish

df_breaks_row = pd.DataFrame({
    "startgroup": STARTGROUPS,
    "Starting": row_start.values,
    "Break-Offs": row_breakoff.values,
    "Break-Offs %": (100 * row_breakoff / row_start).round(2).values,
    "Break-Offs % of Total": (100 * row_breakoff / row_breakoff.sum()).round(2).values
})

# Data Contents Layout
cols = st.columns(3)
with cols[0]:
    total_breakoff = start_participants - finish_participants
    cw.number_card_tworow(total_breakoff,
                          str(round(100 * total_breakoff / start_participants, 2)) + "%",
                          "Total Break-Offs")
with cols[1]:
...
# Filter once for speed analysis
mask_valid_speed = df["avg_speed_kmh"] < MAX_SPEED_KMPH
df_speed_data = df[mask_valid_speed]

# Cache speed aggregations
df_speed = df_speed_data.groupby(["control", "startgroup"])["avg_speed_kmh"].mean().reset_index()
df_speed_gender = df_speed_data.groupby(["control", "gender"])["avg_speed_kmh"].mean().reset_index()
...
# Pre-calculate startgroup averages once
...
# Optimize year-over-year calculations: group once, reuse results
df_unique_participants = df_full.drop_duplicates(subset=["year", "startnr"])

df_start_year = df_unique_participants.groupby(["year", "startgroup"]).size().reset_index(name="startnr")

# Calculate breakoffs more efficiently
df_year_stats = df_unique_participants.groupby("year").size().reset_index(name="starts")
df_finishes = df_full[df_full.control == "Finish"].drop_duplicates(subset=["year", "startnr"]).groupby("year").size().reset_index(name="finish")
df_breaks_year = df_year_stats.merge(df_finishes, on="year")
df_breaks_year["break_offs"] = df_breaks_year["starts"] - df_breaks_year["finish"]
...
cols = st.columns(2)
with cols[0]:
    df_finish_all = df_full[df_full.control == "Finish"]
    fig = px.violin(df_finish_all.sort_values(by="year"), x="year", y="duration_h",
...
with cols[1]:
    fig = px.scatter(df_finish_all, y="placement", x="duration_h", color="year",
...
# Pickers and Headers.
st.header("Individual Results Analysis & Comparison (Year-over-Year):")

# Pre-calculate grouped data once - avoid filtering multiple times
df_full_valid_speed = df_full[df_full.avg_speed_kmh < MAX_SPEED_KMPH]
df_full_avg_startgroup = df_full_valid_speed[df_full_valid_speed.control != "Start"].groupby(["control","startgroup_year"])[["avg_speed_kmh", "d_duration_m", "placement"]].mean().reset_index()
...
cols = st.columns(2)
with cols[0]:
    df_selected = df_full[(df_full.control != "Start") & (df_full.name_startnr_year.isin(selected_names_year))]
    fig = px.line(df_selected, x="control", y="avg_speed_kmh",
                      color="name_startnr_year", title='Average Speed per Section and Year',
                      labels={"avg_speed_kmh": "km/h (avg)", 'control': 'Controlpoint'},
                        color_discrete_sequence=px.colors.qualitative.Set3)

    for i, group in enumerate(selected_groups_year):
        tmp = df_full_avg_startgroup[(df_full_avg_startgroup.startgroup_year==group)]
        fig.add_trace(go.Scatter(x=tmp.control, y=tmp.avg_speed_kmh, name=group,
                                 line=dict(width=2, dash='dot', color=px.colors.qualitative.Prism[i])))

    fig.update_layout(xaxis_type='category')
    fig.update_xaxes(categoryorder='array', categoryarray=CONTROL_POINTS)
    st.plotly_chart(fig, use_container_width=True, config=plotly_config)

with cols[1]:
    fig = px.bar(df_selected, x="control", y="d_duration_m",
...
# Add Placement Information
with cols[0]:
    fig = px.line(df_selected, x="control", y="placement",
